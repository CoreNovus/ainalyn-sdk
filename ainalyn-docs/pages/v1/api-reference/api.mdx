...
| `path` | str | JSON Path-style location of the issue |

### Validation Process

The function performs three validation phases:
...
**Example:**
```python
agent = AgentBuilder("Invalid Name")  # Space
```
...
**Fix:**
```python
agent = AgentBuilder("Valid_Name")  # Letters, numbers, hyphens, underscores
```
...
- [Entities Reference](/v1/api-reference/entities/) - Domain entities including EIP types
